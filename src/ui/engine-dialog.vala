namespace LLMStudio.UI {
    ...
    public class EngineUpdateDialog : Adw.MessageDialog {
        private EngineRelease release;
        private EngineManager engine_manager;

        public signal void update_accepted ();
        public signal void update_skipped  (string skip_tag);

        public EngineUpdateDialog (
            EngineRelease rel, EngineManager mgr, Gtk.Window parent,
            ...
            heading = "Engine Update Available";
            body    = "A new version of <b>llama.cpp</b> is available.\n\n" +
                      "Current version: <b>%s</b>\nNew version: <b>%s</b>".printf (
                          current_version ?? "unknown", rel.tag);
            body_use_markup = true;

            add_response ("skip",        "Skip This Version");
            add_response ("later",       "Remind Me Later");
            add_response ("update",      "Update Now");
        ...
        private void on_response (string resp) {
            switch (resp) {
                case "update":
                    update_accepted ();
                    break;
                case "skip":
                    update_skipped (release.tag);
                    break;
                // "later" → do nothing
            }
        }
